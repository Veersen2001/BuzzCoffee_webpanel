...
function BillingSection() {
    ...
    const handlePlaceOrder = async () => {
        ...
        try {
            ...
            console.log("userdera", orderDetails);
    ...
    // Switch to a new order


    // Determine items to display (order items or cart items)
    ...
    const calculateTotalPrice = () => {
        ...
        if(id_edit_order)
        {
             itemsToDisplay.reduce((total, item) => {
                ...
                const addonsTotal = item.addons
                    ...
                    : 0;
                Total = total + itemBaseTotal + variantsTotal + addonsTotal;
        ...
        cartData.reduce((total, item) => {
            ...
            const addonsTotal = item.addons
                ? item.addons.reduce((sum, addon) => sum + (addon.price || 0), 0) * item.quantity
                : 0;
            return total + itemBaseTotal + variantsTotal + addonsTotal + Total;
        }, 0);
        ...
        <div className="w-full mt-10 px-3 sm:px-6 flex flex-col">
            ...
            <div className="mt-12">
                    ...
                    <div className="overflow-x-auto">
                        <table className="table-auto w-full border-collapse border-gray-300 text-sm">
                            ...
                            <tbody>
                                ...
                                {cartData.length === 0 && !(id_edit_order) ? (
                                  <p>Cart is empty</p>  ) : (
                                    <>
                                        {cartData.map((item) => (
                                            <tr key={item._id}>
                ...
                <div className="max-w-[280px] flex justify-between mt-4">
                    ...
                    <p className="text-md font-semibold text-gray-600">₹{calculateTotalPrice()}</p>
                </div>
            </div>
